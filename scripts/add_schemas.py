...
for i in res.json():
    r = requests.delete(APIURL+"/schemas/"+i["id"], headers=headers)
...
with open("../schemas/landing.json") as o:
    ...
    r = requests.post(APIURL+"/schemas/", headers=headers, json=landing)

for filename in glob("../schemas/ui-schemas/*.json"):
    with open(filename) as o:
        schema = json.loads(o.read())
        schema = create_schema(schema, "/dcic/signature-commons-schema/v5/meta/schema/ui-schema.json")
        r = requests.post(APIURL+"/schemas/", headers=headers, json=schema)
...
with open("../schemas/counts.json") as o:
    ...
    for i in counting_meta:
        ...
        r = requests.post(APIURL+"/schemas/", headers=headers, json=counting_schema)


res = requests.get(APIURL+"/optimize/refresh", headers=headers)
print(res.ok)

res = requests.get(APIURL+"/optimize/status", headers=headers)
